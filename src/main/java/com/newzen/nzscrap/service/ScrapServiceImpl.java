...
public class ScrapServiceImpl implements ScrapService {
	...
	public String scrap() {
		try {
			// 프록시 내용 초기화 -> 이 후 요청별로 다른 proxy 내용을 설정
			proxyService.init();
	 ...
	 * - 요청별 스크래핑 처리 > 요청 inJson 생성 > 요청내용 스크래핑 전송 > 결과 내용(metadata)으로 스크래핑 데이터 생성 
	 * - 스크래핑 히스토리 작업 > 히스토리 요청내용 생성 > 히스토리 응답내용 생성 > 히스토리 DB처리 > 히스토리 비즈북스 전송 
	 * - 스크래핑 데이터 작업 > 데이터 생성 > 데이터 DB처리 > 데이터 비즈북스 전송
	 * 
	 * @param compCd
	...
	private ArrayList<ScrapCertForm> makeScrapCertFormList(String jsonReqParamList) throws IOException {
		...
		for (ServerScrapReqParam reqParam : hometaxReqParamList) {
			...
			case 1: // id
				...
				break;
			case 2: // 인증서
				if (reqParamTmp == null && signCert != null && signCert.length() > 0 && signPri != null
						&& signPri.length() > 0) {
		...
		for (ServerScrapReqParam reqParam : cardsalesReqParamList) {
			...
			if (reqParamTmp == null && loginId != null && loginId.length() > 0 && loginPw != null
					&& loginPw.length() > 0) {
				certMap.put(svcCd, reqParam);
			}
		}
			...
			try {
				...
				case "hometax": // 홈택스
					...
					case "hometaxZ300X": // 전자세금계산서
						...
						// 201211_kmh 홈택스_전자세금계산서 전송시 proxy 제외
						inJsonHometax.setProxy("");
					...
					case "hometaxZ40XX": // 현금영수증, 사업용(화물복지)신용카드
						...
						// 등록된 홈택스 id가 있는 경우
						if (inJsonHometax.getUserId() != null && inJsonHometax.getUserId().isEmpty() == false) {
							// id 제외 기타 인증내용 제거
	...
	private void setScrapCertFormHistoryRes(ArrayList<ScrapCertForm> scrapCertFormList) {
		...
		try {
			for (ScrapCertForm scrapCertForm : scrapCertFormList) {
				...
				ArrayList<HashMap<String, Object>> outJsonList = (ArrayList<HashMap<String, Object>>) resParam
						.get("outJsonList");
	 ...
	 * 
	 * 비즈북스에 인증 오류내용을 전송합니다. 비즈북스에서는 이 후 해당 항목이 재 스크랩 되지 않도록 스크랩 사용여부를 비활성화 처리합니다.
	 * 
	 ...
	 */
	private ArrayList<ServerScrapReqParam> makeReqParamList(ArrayList<ScrapCertForm> scrapCertFormList,
			String jsonReqParamList) {
		ObjectMapper objectMapper = new ObjectMapper();
		ArrayList<ServerScrapReqParam> reqParamList = null;

		try {
			reqParamList = objectMapper.reader().forType(new TypeReference<ArrayList<ServerScrapReqParam>>() {
			}).readValue(jsonReqParamList);
			...
			for (ScrapCertForm scrapCertForm : scrapCertFormList) {
				...
				for (ServerScrapReqParam reqParam : reqParamList) {
					...
					case "bank":
					case "sbk":
						if (certSvcCd.equals("accountCbk") || certSvcCd.equals("accountBank")
								|| certSvcCd.equals("accountSbk")) {
							if (reqParam.getKeyCd() == historyRes.getKeyCd()) {
								isErr = historyRes.getErrYn().equals("Y");
								reqParam.setErrYn(isErr ? "Y" : "N");
	 ...
	 */
	private void sendCertFailedHistoryList(ArrayList<ScrapCertForm> scrapCertFormList,
	 ...
	 * sendDataToBizbooks
	 * 
	 * 비즈북스에 데이터를 전송합니다. 데이터는 스크래유형에 따라 단일, 복수건에 따라 다르게 처리합니다. 
	 * - 단일 : 바로 전송 
	 * - 복수 : > (데이터 / DATA_DIVISION_LEN) 횟수로 DELAY_MS 시간마다 분할 전송 > 스크래핑 유형에 따라 delete -> insert 처리하는 항목있음 (A.I field 사용하는 항목들)
	 * 
				...
				if (entry.getValue() instanceof ArrayList<?>) {
					...
					if (size > 0) {
						...
						for (int i = 0; i <= cnt; i++) {
							...
							int endIdx = size <= ((i * DATA_DIVISION_LEN) + DATA_DIVISION_LEN) ? size
									: ((i * DATA_DIVISION_LEN) + DATA_DIVISION_LEN);
	...
	private void insertHistoryResList(String jsonResList) {
		...
		try {
			historyResList = objectMapper.reader().forType(new TypeReference<ArrayList<HistoryRes>>() {
	...
	public String insertData(String dataType, String jsonData) {
		...
		try {
			...
			// - 전자세금계산서
			case "hometaxZ300X":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ300X>>() {
				}).readValue(jsonData);
			...
			case "hometaxZ300XItem":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ300XItem>>() {
				}).readValue(jsonData);
			...
			// - 현금영수증
			case "hometaxZ4001":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4001>>() {
				}).readValue(jsonData);
				...
				break;
			case "hometaxZ4002":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4002>>() {
				}).readValue(jsonData);
			...
			// - 사업용 화물복지 신용카드
			case "hometaxZ4010":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4010>>() {
				}).readValue(jsonData);
				...
				break;
			case "hometaxZ4020":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4020>>() {
				}).readValue(jsonData);
			...
			// - 신용카드 매출자료 조회
			case "hometaxZ4050":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4050>>() {
				}).readValue(jsonData);
			...
			// - 현금영수증 매출총액 조회
			case "hometaxZ4060":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4060>>() {
				}).readValue(jsonData);
			...
			// - 수출실적명세서 조회
			case "hometaxZ4070":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ4070>>() {
				}).readValue(jsonData);
			...
			// - 세금신고 접수증조회
			case "hometaxZ5002":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ5002>>() {
				}).readValue(jsonData);
			...
			// - 부가세신고용 합계표
			case "hometaxZ0006":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ0006>>() {
				}).readValue(jsonData);
			...
			case "hometaxZ0006Dtl":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SHometaxZ0006Dtl>>() {
				}).readValue(jsonData);
			...
			// - 가맹점 수수료율/대금지급주기
			case "cardsalesB0002":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0002>>() {
				}).readValue(jsonData);
			...
			// - 기간별승인내역(일별)
			case "cardsalesB0011Dtl":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0011Dtl>>() {
				}).readValue(jsonData);
			...
			case "cardsalesB0011Sum":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0011Sum>>() {
				}).readValue(jsonData);
			...
			case "cardsalesB0021Dtl":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0021Dtl>>() {
				}).readValue(jsonData);
			...
			case "cardsalesB0021Sum":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0021Sum>>() {
				}).readValue(jsonData);
			...
			// - 기간별입금내역
			case "cardsalesB0031Dtl":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0031Dtl>>() {
				}).readValue(jsonData);
			...
			case "cardsalesB0031Sum":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardsalesB0031Sum>>() {
				}).readValue(jsonData);
			...
			// - 개인
			case "bankB0002":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SBankB0002>>() {
				}).readValue(jsonData);
			...
			// - 법인
			case "cbkB0002":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCbkB0002>>() {
				}).readValue(jsonData);
			...
			// - 빠른조회
			case "sbkB0002":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SSbkB0002>>() {
				}).readValue(jsonData);
			...
			// - 개인
			case "cardC0005":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCardC0005>>() {
				}).readValue(jsonData);
			...
			// - 법인
			case "ccdC0005":
				dataList = objectMapper.reader().forType(new TypeReference<ArrayList<SCcdC0005>>() {
				}).readValue(jsonData);
				...
				if (historyRes.getErrYn().equals("N")) {
					...
					case "hometax":
						...
						case "Z3004":
							...
							dataListMap.put("hometaxZ300XItem",
									scrapMapper.selectSHometaxZ300XItemList(sHometaxZ300XItemParam));
							break;
						// - 현금영수증
						case "Z4001":
						...
						case "Z0006":
							...
							dataListMap.put("hometaxZ0006Dtl",
									scrapMapper.selectSHometaxZ0006DtlList(sHometaxZ0006DtlParam));
							break;
						}
						break;
					// 여신
					case "cardsales":
						...
						case "B0002":
							...
							dataListMap.put("cardsalesB0002",
									scrapMapper.selectSCardsalesB0002List(sCardsalesB0002Param));
							break;
						// - 기간별승인내역(일별)
						case "B0011":
							...
							dataListMap.put("cardsalesB0011Dtl",
									scrapMapper.selectSCardsalesB0011DtlList(sCardsalesB0011DtlParam));
							...
							dataListMap.put("cardsalesB0011Sum",
									scrapMapper.selectSCardsalesB0011SumList(sCardsalesB0011SumParam));
							break;
						// - 기간별매입내역(일별)
						case "B0021":
							...
							dataListMap.put("cardsalesB0021Dtl",
									scrapMapper.selectSCardsalesB0021DtlList(sCardsalesB0021DtlParam));
							...
							dataListMap.put("cardsalesB0021Sum",
									scrapMapper.selectSCardsalesB0021SumList(sCardsalesB0021SumParam));
							break;
						// - 기간별입금내역
						case "B0031":
							...
							dataListMap.put("cardsalesB0031Dtl",
									scrapMapper.selectSCardsalesB0031DtlList(sCardsalesB0031DtlParam));
							...
							dataListMap.put("cardsalesB0031Sum",
									scrapMapper.selectSCardsalesB0031SumList(sCardsalesB0031SumParam));
							break;
						}
						break;
					// 뱅킹_개인
	...
	private HashMap<String, Object> makeInsertData(String jsonOutJsonList, String jsonResParamList) throws IOException {
		...
		ArrayList<HashMap<String, Object>> outJsonList = objectMapper.reader()
				.forType(new TypeReference<ArrayList<HashMap<String, Object>>>() {
				}).readValue(jsonOutJsonList);

		ArrayList<HistoryRes> resParamList = objectMapper.reader().forType(new TypeReference<ArrayList<HistoryRes>>() {
		}).readValue(jsonResParamList);

		if ((outJsonList == null || resParamList == null) || (outJsonList.size() != resParamList.size())) {
			return null;
		}
		...
		for (int i = 0; i < outJsonList.size(); i++) {
			try {
				...
				case "hometax":
					...
					case "Z3004": {
						// -- 내역
						ArrayList<SHometaxZ300X> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ300X>>() {
								});
						...
						// -- 품목
						ArrayList<SHometaxZ300XItem> itemList = objectMapper.convertValue(outJson.get("itemList"),
								new TypeReference<ArrayList<SHometaxZ300XItem>>() {
								});
					...
					// - 현금영수증
					case "Z4001": {
						ArrayList<SHometaxZ4001> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4001>>() {
								});
						if (list != null && list.size() > 0) {
					...
					case "Z4002": {
						ArrayList<SHometaxZ4002> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4002>>() {
								});
						if (list != null && list.size() > 0) {
					...
					// - 사업용 화물복지 신용카드
					case "Z4010": {
						ArrayList<SHometaxZ4010> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4010>>() {
								});
						if (list != null && list.size() > 0) {
					...
					case "Z4020": {
						ArrayList<SHometaxZ4020> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4010>>() {
								});
						if (list != null && list.size() > 0) {
					...
					// - 신용카드 매출자료 조회
					case "Z4050": {
						ArrayList<SHometaxZ4050> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4050>>() {
								});
						if (list != null && list.size() > 0) {
					...
					// - 현금영수증 매출총액 조회
					case "Z4060": {
						ArrayList<SHometaxZ4060> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4060>>() {
								});
						if (list != null && list.size() > 0) {
					...
					// - 수출실적명세서 조회
					case "Z4070": {
						ArrayList<SHometaxZ4070> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ4070>>() {
								});
						if (list != null && list.size() > 0) {
					...
					// - 세금신고 접수증조회
					case "Z5002": {
						ArrayList<SHometaxZ5002> list = objectMapper.convertValue(outJson.get("list"),
								new TypeReference<ArrayList<SHometaxZ5002>>() {
								});
					...
					case "Z0006": {
						ArrayList<HashMap<String, Object>> outZ0006List = objectMapper.convertValue(
								outJson.get("outZ0006"), new TypeReference<ArrayList<HashMap<String, Object>>>() {
								});
						...
						if (outZ0006List != null) {
							for (HashMap<String, Object> outZ0006 : outZ0006List) {
								...
								// SHometaxZ0006 sHometaxZ0006 = objectMapper.convertValue(outZ0006,
								// SHometaxZ0006.class);
								...
								sHometaxZ0006
										.setWithIn_bsnoIsnClplcCnt(outZ0006.get("WithIn_bsnoIsnClplcCnt").toString());
								...
								sHometaxZ0006
										.setWithIn_resnoIsnClplcCnt(outZ0006.get("WithIn_resnoIsnClplcCnt").toString());
								sHometaxZ0006.setWithIn_resnoIsnScnt(outZ0006.get("WithIn_resnoIsnScnt").toString());
								sHometaxZ0006
										.setWithIn_resnoIsnSplCft(outZ0006.get("WithIn_resnoIsnSplCft").toString());
								...
								sHometaxZ0006
										.setWithIn_totPurchplcCnt(outZ0006.get("WithIn_totPurchplcCnt").toString());
								...
								ArrayList<SHometaxZ0006Dtl> sHometaxZ0006DtlEtList = objectMapper.convertValue(
										outZ0006.get("listEt"), new TypeReference<ArrayList<SHometaxZ0006Dtl>>() {
										});
								ArrayList<SHometaxZ0006Dtl> sHometaxZ0006DtlWithInList = objectMapper.convertValue(
										outZ0006.get("listWithIn"), new TypeReference<ArrayList<SHometaxZ0006Dtl>>() {
										});
				...
				case "cardsales":
					...
					// - 가맹점 수수료율/대금지급주기
					case "B0002": {
						HashMap<String, Object> outB0002 = objectMapper.convertValue(outJson.get("outB0002"),
								new TypeReference<HashMap<String, Object>>() {
								});

						ArrayList<SCardsalesB0002> list = objectMapper.convertValue(outB0002.get("list"),
								new TypeReference<ArrayList<SCardsalesB0002>>() {
								});
					...
					case "B0011": {
						// -- 상세
						HashMap<String, Object> outB0011 = objectMapper.convertValue(outJson.get("outB0011"),
								new TypeReference<HashMap<String, Object>>() {
								});

						ArrayList<SCardsalesB0011Dtl> listDtl = objectMapper.convertValue(outB0011.get("listDtl"),
								new TypeReference<ArrayList<SCardsalesB0011Dtl>>() {
								});
						...
						// -- 합계
						ArrayList<SCardsalesB0011Sum> listSum = objectMapper.convertValue(outB0011.get("listSum"),
								new TypeReference<ArrayList<SCardsalesB0011Sum>>() {
								});
					...
					case "B0021": {
						// -- 상세
						HashMap<String, Object> outB0021 = objectMapper.convertValue(outJson.get("outB0021"),
								new TypeReference<HashMap<String, Object>>() {
								});

						ArrayList<SCardsalesB0021Dtl> listDtl = objectMapper.convertValue(outB0021.get("listDtl"),
								new TypeReference<ArrayList<SCardsalesB0021Dtl>>() {
								});
						...
						// -- 합계
						ArrayList<SCardsalesB0021Sum> listSum = objectMapper.convertValue(outB0021.get("listSum"),
								new TypeReference<ArrayList<SCardsalesB0021Sum>>() {
								});
					...
					// - 기간별입금내역
					case "B0031": {
						HashMap<String, Object> outB0031 = objectMapper.convertValue(outJson.get("outB0031"),
								new TypeReference<HashMap<String, Object>>() {
								});

						// -- 상세
						ArrayList<SCardsalesB0031Dtl> listDtl = objectMapper.convertValue(outB0031.get("listDtl"),
								new TypeReference<ArrayList<SCardsalesB0031Dtl>>() {
								});
						...
						// -- 합계
						ArrayList<SCardsalesB0031Sum> listSum = objectMapper.convertValue(outB0031.get("listSum"),
								new TypeReference<ArrayList<SCardsalesB0031Sum>>() {
								});
				...
				case "bank":
					...
					// - 거래내역조회 (입출금통장)
					case "B0002": {
						ArrayList<HashMap<String, Object>> outB0002List = objectMapper.convertValue(
								outJson.get("outB0002"), new TypeReference<ArrayList<HashMap<String, Object>>>() {
								});
						...
						ArrayList<SBankB0002> list = objectMapper.convertValue(outB0002.get("list"),
								new TypeReference<ArrayList<SBankB0002>>() {
								});
				...
				case "cbk":
					...
					case "B0002": {
						...
						ArrayList<SCbkB0002> list = objectMapper.convertValue(outB0002.get("list"),
								new TypeReference<ArrayList<SCbkB0002>>() {
								});
				...
				case "sbk":
					...
					case "B0002": {
						...
						ArrayList<SSbkB0002> list = objectMapper.convertValue(outB0002.get("list"),
								new TypeReference<ArrayList<SSbkB0002>>() {
								});
				...
				case "card":
					...
					// - 승인내역조회
					case "C0005": {
						HashMap<String, Object> outC0005 = objectMapper.convertValue(outJson.get("outC0005"),
								new TypeReference<HashMap<String, Object>>() {
								});

						ArrayList<SCardC0005> list = objectMapper.convertValue(outC0005.get("list"),
								new TypeReference<ArrayList<SCardC0005>>() {
								});
				...
				case "ccd":
					...
					case "C0005": {
						...
						ArrayList<SCcdC0005> list = objectMapper.convertValue(outC0005.get("list"),
								new TypeReference<ArrayList<SCcdC0005>>() {
								});
	...
	/**
	 * getBizbooksScrapSchedule - 비즈북스에서 스크래핑을 수행할 내역들을 가져옵니다. 이 후 스크래핑은
	 * 회사코드(CompCd)별로 수행됩니다.
	 * 
	 * TODO: 기간별, 모듈별 스크래핑 수행토록 수정 필요
	 * - 현재 작성된 스크래핑 일시 상관없이 스케쥴러 내용 기간으로만 스크래핑됨
	 * (201214 기준) 
	 *  
	 * 
	...
	/**
	 * getBizbooksScrapListMap
	 * 
	 * 비즈북스 사이트에서 조회기간에 해당하는 스크래핑 리스트를 가져옵니다.
	 * 
	 * TODO: 기간별, 모듈별 스크래핑 수행토록 수정 필요
	 * - 현재 작성된 스크래핑 일시 상관없이 스케쥴러 내용 기간으로만 스크래핑됨
	 * (201214 기준) 
	...
	/**
	 * getDefaultScrapDt - 스크래핑을 수행할 기본 날짜를 설정합니다
	 * 
	 * @return HashMap<String, String> 
	 * - fromDt: 스크래핑 조회 시작일자 
	 * - toDt: 스크래핑 조회 종료일자
	...
	private HashMap<String, Object> makeInsertData_test(HashMap<String, Object> outJson, ServerScrapReqParam reqParam)
			throws IOException {
		ObjectMapper objectMapper = new ObjectMapper();
		...
		try {
			...
			case "hometax":
				...
				case "Z0006": {
					...
					ArrayList<HashMap<String, Object>> outZ0006List = objectMapper.convertValue(outJson.get("outZ0006"),
			...
			case "bank":
				...
				case "B0002": {
					...
					ArrayList<HashMap<String, Object>> outB0002List = objectMapper.convertValue(outJson.get("outB0002"),
							new TypeReference<ArrayList<HashMap<String, Object>>>() {
							});
	 ...
	 */
	private HistoryRes getErrorHistoryRes(ArrayList<ScrapCertForm> scrapCertFormList,
			ServerScrapReqParam serverScrapReqParam) {
		...
		for (ScrapCertForm scrapCertForm : scrapCertFormList) {
			...
			if (orgCd.equals(historyRes.getOrgCd())) {
				...
				case "card":
				case "ccd":
					if ((certSvcCd.equals("creditCcd") || certSvcCd.equals("creditCard")) && errYn.equals("Y")
							&& keyCd == historyRes.getKeyCd()) {
						historyRes.setSvcCd(svcCd);
						historyRes.setReqCd(reqCd);
						return historyRes;
					}
				...
				case "cbk":
				case "sbk":
					if ((certSvcCd.equals("accountBank") || certSvcCd.equals("accountCbk")
							|| certSvcCd.equals("accountSbk")) && errYn.equals("Y") && keyCd == historyRes.getKeyCd()) {
						historyRes.setSvcCd(svcCd);
						historyRes.setReqCd(reqCd);
						return historyRes;
					}
	...
	/**
	 * scrapByCompCd_old (TODO: Deprecated) - 회사코드 별로 스크래핑업무를 수행합니다. (요청 일괄처리)
	 * 
	 * @param compCd
	 * @param jsonReqParamList
	 * 
